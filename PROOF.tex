...
\((K-d-1)/2\) of these intervals satisfy \(|p|>H\) at every point.
\end{lemma}
...
Let \(0<t<1\), and choose \(a_t<b<1\).
Let \(s\to\infty\), \(m=\lfloor ts\rfloor\), and let \(Q_s\) be real
on \([-b,b]\) and analytic in a fixed complex neighborhood of that
interval. Assume that \(Q_s\) is sufficiently close to \(Q\) there
in the uniform norm.
The weight \(e^{-2sQ_s}\), with \(m\) orthogonal polynomials, corresponds
to the external field \(2sQ_s/m\) in \cite{KSSV}.
The limiting field \(2Q/t\) satisfies hypothesis (GA)\(_2\) of that
paper: it is analytic and strictly convex, and its equilibrium
support is contained in the interior of \([-b,b]\).
Theorem 1.3(b) of \cite{KSSV} therefore gives estimates uniform in
the indicated neighborhood of \(Q\).

Let \(p_0,\ldots,p_{m-1}\) be orthonormal for
\(e^{-2sQ_s(x)}\,dx\) on \([-b,b]\), and set
...
\end{equation}
Put \(A=[-1/4,1/4]\).
For \(y\in A\) and bounded real \(u,v\), \cite[Theorem 1.8 (bulk universality)]{KSSV} gives
\begin{equation}\label{eq:sine-limit}
 \frac{K_s(y+u/c_s,y+v/c_s)}{c_s}
 =
 \frac{\sin\bigl(\pi\beta_{s,y}(u-v)\bigr)}{\pi(u-v)}
 +o(1).
\end{equation}
Here \(\beta_{s,y}\) is twice the density at \(y\) of the
equilibrium measure of mass \(m/s\) for \(Q_s\), and at \(u=v\)
the quotient on the right is interpreted by continuity.
To match the normalization in \cite[Theorem 1.8 (bulk universality)]{KSSV}, set
\[
 V_s=\frac{2s}{m}Q_s,
\]
so that \(e^{-mV_s}=e^{-2sQ_s}\). If \(\psi_s\) is the equilibrium
probability density for \(V_s\), then
\(\rho_s=(m/s)\psi_s\) is the equilibrium density of mass \(m/s\)
for \(Q_s\), and hence
\[
 m\psi_s(y)=s\rho_s(y)=c_s\beta_{s,y}.
\]
Substituting the local variables \(\beta_{s,y}u\) and
\(\beta_{s,y}v\) in \cite[Theorem 1.8 (bulk universality)]{KSSV} gives
\eqref{eq:sine-limit}. The error is uniform for \(y\in A\), bounded
\(u,v\), and the indicated analytic neighborhood of \(Q\), by part
(b) of that theorem.

We also need estimates away from the diagonal.
Choose fixed intervals \(J_0\Subset J_1\) in the common interior of
the equilibrium supports, with \(A\Subset J_0\).
The difference-quotient formula in
\cite[Theorem 1.3]{KSSV} gives
\begin{equation}\label{eq:bulk-kernel}
 \frac{|K_s(x,z)|}{c_s}
 \le \frac{C}{1+s|x-z|}
 \qquad(x,z\in J_1).
\end{equation}
Near the diagonal, this follows from the diagonal estimate and
Cauchy--Schwarz.
For \(z\) within \(O(s^{-1})\) of \(A\), the Airy formulas in the
same theorem give
\begin{equation}\label{eq:edge-kernel}
 \frac{|K_s(x,z)|}{c_s}\le Cs^{-5/6}
 \qquad(x\in[-b,b]\setminus J_0).
\end{equation}
Indeed, the kernel factors at \(z\) are bounded, those near a support
endpoint are at most \(O(s^{1/6})\), and \(x\) and \(z\) are separated
by a fixed positive distance.
The constants in \eqref{eq:bulk-kernel} and \eqref{eq:edge-kernel}
are uniform for the external fields under consideration.
...
measure, \eqref{eq:cardinal-growth} and Remez's inequality yield
\[
...
Now let \(x_0\) be a density point of \(E\) and a Lebesgue point
of the absolutely continuous part of \(\nu\), with finite positive
density \(w\); assume also that the singular part has density zero
there. These conditions hold at almost every point of \(E\) where
the density of \(\nu\) is positive.
Put
\[
 I_h=(x_0-h,x_0+h),\qquad
 a_h=\nu(I_h)\sim2wh,
\]
and let \(\mu_h\) be the image of \(\nu|_{I_h}/a_h\) under
\(x\mapsto(x-x_0)/h\).
Then \(\mu_h\Rightarrow du/2\) on \([-1,1]\), so
...
\((-1,1)\).
The left side of \eqref{eq:outer-identity} is convex.
Consequently \eqref{eq:inner-potential} and
\eqref{eq:outer-identity} imply convergence in measure to \(Q-1\),
and hence uniform convergence on every compact subinterval.
Subtracting the value at zero yields
\begin{equation}\label{eq:real-field-limit}
 A_h(u)-A_h(0)\longrightarrow Q(u)
 \quad\text{locally uniformly on }(-1,1).
\end{equation}

We recall the convexity fact used here.
If finite convex functions on an open interval converge in measure
to a continuous finite convex function, they converge uniformly on
compact subintervals.
Indeed, from any subsequence one can extract an almost everywhere
convergent subsequence. Convergence points on either side of a given
compact interval bound the secant slopes. Convexity then gives
equicontinuity on that interval, and the limit is fixed by
convergence in measure.

The convergence in \eqref{eq:real-field-limit} also holds in a fixed
complex neighborhood of each compact subinterval of \((-1,1)\).
...
distance from \((-\infty,-1]\cup[1,\infty)\). Write
\(\xi=(y-x_0)/h\); for \(y\notin I_h\) one has \(|\xi|\ge1\). Define
...
\(\widetilde A_h(u)=A_h(u)-A_h(0)\). Moreover,
\[
 \widetilde A_h''(z)
 =\frac1{a_h}\int_{\R\setminus I_h}
   \frac{d\nu(y)}{(z-\xi)^2},
\]
so \(A_h''(u)=\widetilde A_h''(u)>0\) on the real interval.

For sufficiently small fixed \(\ell>0\), the interval
\([u-\ell,u+\ell]\) stays in a fixed compact subinterval of
\((-1,1)\) for every \(u\in K_1\). Since \(|\xi|\ge1\), the kernels
\((u-\xi)^{-2}\) are uniformly comparable on such intervals. Hence
\[
 A_h''(u)
...
convexity give uniform interior bounds for \(A_h'\), and hence a
uniform bound for \(A_h''\) on \(K_1\). If \(z\) lies in a
sufficiently thin complex neighborhood of \(K_1\), then
\[
 |z-\xi|^{-2}\le|\operatorname{Re}z-\xi|^{-2},
\]
and the same bound holds for \(|\widetilde A_h''(z)|\).
Together with \(\widetilde A_h(0)=0\) and the uniform interior bound
on \(A_h'(0)\), this gives a normal family on a fixed complex
neighborhood of \(K_1\). Every subsequential limit agrees with \(Q\)
on the real interval by \eqref{eq:real-field-limit}, hence equals
\(Q\) by the identity theorem. Thus \(\widetilde A_h\to Q\) locally
uniformly near \(K\).

Apply this with \(K=[-b,b]\). Choose \(h\) small enough that
\(\widetilde A_h\) belongs to the analytic neighborhood required in
Proposition~\ref{prop:weighted} and
\[
 \mu_h(A)<\frac14+\frac{\epsilon_\rho}{2}.
\]
This is possible because \(\mu_h\Rightarrow du/2\) and
\((du/2)(A)=1/4\). Choose \(h\) so that the endpoints of \(I_h\) and
the preimages of the endpoints of \(A\) are not atoms of \(\nu\),
and keep it fixed. For the fixed finite set \(S\) above, put
\[
 Y_n=X_n\setminus S,\qquad s_n=\#(Y_n\cap I_h).
\]
Weak convergence then gives
\[
 \frac{s_n}{n}\longrightarrow a_h>0,
 \qquad
 \frac{\#\{y\in Y_n:(y-x_0)/h\in A\}}{s_n}
 \longrightarrow\mu_h(A)<\frac14+\frac{\epsilon_\rho}{2}.
\]
In particular, \eqref{eq:central-count} holds for all sufficiently
large \(n\). Let \(\mathcal Q_n\) denote the normalized analytic
continuation of the discrete field \eqref{eq:actual-field}, so that
\(\mathcal Q_n(0)=0\) and
\[
 \mathcal Q_n'(z)
 =-\frac{h}{s_n}\sum_{y\in Y_n\setminus I_h}
   \frac1{x_0+hz-y}.
\]
On the fixed complex neighborhood used above, the summands are
uniformly bounded and equicontinuous as functions of the outer node.
Since \(n/s_n\to1/a_h\), weak convergence of the empirical measures
gives \(\mathcal Q_n'\to\widetilde A_h'\) locally uniformly. Integrating
from zero yields \(\mathcal Q_n\to\widetilde A_h=A_h-A_h(0)\) locally
uniformly in the same neighborhood. If \(N_n=\#Y_n\), then the
allowed degree \(n+r_n\) is \(N_n+d_n\), where
\[
 d_n=r_n+\#(X_n\cap S)=o(s_n).
\]
Proposition~\ref{prop:weighted} therefore applies.
