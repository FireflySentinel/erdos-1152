...
For \eqref{eq:bulk-kernel}, use the difference-quotient formula
\cite[Theorem~1.3(a), (1.25)]{KSSV}. Near the diagonal the same
estimate follows from the diagonal bound and Cauchy--Schwarz.
...
of \(\nu\) is positive. Since \(\overline\Omega\Subset\Omega_0\),
Lemma~\ref{lem:local-outer-field} gives
\(\norm{\widetilde A_h-Q}_\Omega\to0\). Choose
\(h\) so small that this norm is less than \(\delta/2\), where
\(\mathcal U\) is the neighborhood fixed in Proposition~\ref{prop:weighted}, and
\[
...
\(\norm{\mathcal Q_n-\widetilde A_h}_\Omega<\delta/2\) for all
sufficiently large \(n\), so \(\mathcal Q_n\in\mathcal U\).
...
It covers Lemma~\ref{lem:signs}, the scalar calculations of
Lemma~\ref{lem:equilibrium}, the deduction from
